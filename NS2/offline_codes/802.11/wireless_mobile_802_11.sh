...
do
    ...
    ns wireless_mobile_802_11.tcl $nNodes $nFlows $pcktRate $speed
    #echo "---------------------------------------------------------------------" >> $output_file
...
do
    ...
    for((i=1;i<7;i++))
    do
	gnuplot -persist -e "set title '802.11 : ${arr[$i]} vs No of flows'; set xlabel 'No of Flows'; set ylabel '${arr2[$i]}'; plot 'out/GRAPH' using 1:$i+1 with lines"
    done
    truncate -s 0 $graphData
 ...
 #variation in number of packets
 for((i=0;i<5;i++))
do
    ...
    for((i=1;i<7;i++))
    do
	gnuplot -persist -e "set title '802.11 : ${arr[$i]} vs No of Packet Rates'; set xlabel 'Packet Rate'; set ylabel '${arr2[$i]}'; plot 'out/GRAPH' using 1:$i+1 with lines"
    done
    truncate -s 0 $graphData
...
do
    ...
    ns wireless_mobile_802_11.tcl $nNodes $nFlows $pcktRate $spd
    #echo "---------------------------------------------------------------------" >> $output_file
    #echo "iteration: $i, node: ${nodes[$i]}" >> $output_file
    awk -f wireless_mobile_802_11.awk wireless_mobile_802_11.tr >> $output_file
    #echo "---------------------------------------------------------------------" >> $output_file
    ...
    for((i=1;i<7;i++))
    do
	gnuplot -persist -e "set title '802.11 : ${arr[$i]} vs Speed'; set xlabel 'Speed'; set ylabel '${arr2[$i]}'; plot 'out/GRAPH' using 1:$i+1 with lines"
    done
    truncate -s 0 $graphData
